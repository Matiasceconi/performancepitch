...
export default function RivalClubSearch({ clubs = [], value = "", selectedClubId = "", onSelect, onCreated, label = "Rival" }) {
  ...
  const results = useMemo(() => (query ? clubs.filter((club) => club.active !== false && clubMatchesQuery(club, query)).slice(0, 8) : []), [clubs, query]);
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="relative">
        ...
        <input className="w-full bg-zinc-800 border border-zinc-700 rounded-lg py-2 pl-9 pr-3 text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-zinc-500" placeholder="Buscar club..." value={query} onChange={(e) => setQuery(e.target.value)} />
      </div>
      {selected && <div className="flex items-center gap-2 rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 text-xs text-zinc-300">{selected.shield_url && <img src={selected.shield_url} alt={selected.official_name} className="h-7 w-7 object-contain" />}<span>{selected.official_name}</span>{selected.short_name && <span className="text-zinc-500">({selected.short_name})</span>}</div>}
      {(results.length > 0 || query) && <div className="rounded-xl border border-zinc-800 bg-zinc-950 overflow-hidden">
