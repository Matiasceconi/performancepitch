...
import { AlertCircle, ArrowUpDown, Expand, TrendingDown, TrendingUp, X } from "lucide-react";
...
export default function SessionGpsReportContent({ session, reportData, observations, setObservations, saving, onSaveObservations }) {
  ...
  return (
    <div className="gps-report-export bg-zinc-900 text-white space-y-6 p-1">
      ...
      {reportData.alerts.length > 0 && <section className="space-y-2"><h3 className="text-sm font-bold">7. Alertas</h3>{reportData.alerts.map((a, i) => <div key={i} className="flex items-center gap-2 px-3 py-1.5 bg-red-500/5 border border-red-500/20 rounded-lg"><AlertCircle size={11} className="text-red-400 shrink-0" /><span className="text-xs text-red-300">{a.text}</span></div>)}</section>}
      <section className="space-y-2"><h3 className="text-sm font-bold">8. Observaciones</h3><textarea rows={3} value={observations} onChange={e => setObservations(e.target.value)} placeholder="Observaciones del cuerpo técnico sobre esta sesión..." className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-xs text-white resize-none focus:outline-none focus:border-zinc-500" /><button onClick={onSaveObservations} disabled={saving} className="px-3 py-1.5 bg-zinc-800 border border-zinc-700 text-zinc-300 hover:bg-zinc-700 rounded-lg text-xs transition-colors disabled:opacity-50">{saving ? "Guardando..." : "Guardar observaciones"}</button></section>
