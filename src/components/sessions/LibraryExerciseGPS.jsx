...
export default function LibraryExerciseGPS({ libraryExerciseId }) {
  ...
  useEffect(() => {
    base44.entities.LibraryExerciseGPSData
      .filter({ library_exercise_id: libraryExerciseId }, "-session_date", 1000)
      .then(data => { setRows(data); setLoading(false); });
  }, [libraryExerciseId]);

  if (loading) return <div className="flex justify-center py-4"><div className="w-4 h-4 border-2 border-zinc-700 border-t-white rounded-full animate-spin" /></div>;

  if (rows.length === 0) {
    return (
      <div className="py-6 text-center">
        <Activity size={20} className="text-zinc-700 mx-auto mb-2" />
        <p className="text-xs text-zinc-600">Sin carga externa histórica cargada.</p>
        <p className="text-[10px] text-zinc-700 mt-1">Cargá un CSV en un ejercicio de sesión vinculado a esta biblioteca para ver datos aquí.</p>
      </div>
    );
  }
  ...
  // Unique loads (by session+exercise)
  const loadKeys = [...new Set(filtered.map(r => `${r.session_id}__${r.exercise_id}`))];
  const totalPlayers = new Set(filtered.map(r => r.player_id)).size;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-2.5 text-center">
          <p className="text-base font-bold text-emerald-400">{totalPlayers}</p>
          <p className="text-[9px] text-zinc-500">Jugadores</p>
        </div>
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-2.5 text-center">
          <p className="text-base font-bold text-amber-400">{filtered.length}</p>
          <p className="text-[9px] text-zinc-500">Registros totales</p>
        </div>
      </div>
