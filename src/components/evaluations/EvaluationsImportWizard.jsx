  ...
  async function handleDryRun() {
    ...
    try {
      ...
      const data = await importEvaluations({
        ...
        fallback_date: fallbackDate || null,
        context,
        files: uploaded,
      });
      setPreview(data);
  ...
  const content = <div className="space-y-5">
    ...
    {step === 1 && <>
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        <div><label className="text-xs text-zinc-500 block mb-1">Fecha de respaldo (solo si el CSV no trae fecha)</label><input type="date" value={fallbackDate} onChange={(event) => setFallbackDate(event.target.value)} className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-2.5 py-2 text-sm text-white" /></div>
