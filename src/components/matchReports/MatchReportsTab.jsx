...
import { FileText, Plus, Download, Send, Eye, Loader2, Inbox, X } from "lucide-react";
import { base44 } from "@/api/base44Client";
import { useWorkspace } from "@/lib/WorkspaceContext";
import PlayerPhoto from "@/components/player/PlayerPhoto";
import MatchReportBuilder from "./MatchReportBuilder";
...
export default function MatchReportsTab() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const rows = await base44.entities.PlayerMatchReport.filter({ squad_id: activeSquad.id }, "-created_at", 200);
      setReports(rows);
    } catch {
      setReports([]);
    } finally {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-bold text-white">Reportes individuales de partido</h2>
          <p className="text-xs text-zinc-500">Informes GPS profesionales para cada futbolista</p>
        </div>
        <button onClick={() => setShowBuilder(true)} className="px-4 py-2 rounded-lg bg-emerald-600 text-white text-sm font-semibold flex items-center gap-2">
      ...
      {loading ? (
        ...
        <div className="py-12 text-center">
          ...
          <p className="text-zinc-500 text-sm">No hay reportes generados todavía.</p>
          <p className="text-zinc-600 text-xs mt-1">Creá el primero con "Nuevo reporte".</p>
        </div>
      ) : (
              ...
              <div key={r.id} className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4">
                <div className="flex items-start justify-between gap-2 mb-3">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-zinc-500">{r.player_name} · {r.report_type === "multi_match" ? "Multi-partido" : "Un partido"}</p>
                  </div>
      ...
      {showBuilder && (
        <MatchReportBuilder
          embedded={false}
          onClose={() => { setShowBuilder(false); load(); }}
          onSaved={load}
        />
      )}
