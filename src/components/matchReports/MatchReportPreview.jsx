  ...
  return (
    <div className="space-y-4">
      ...
      {config.showProfileComparison && hasProfile && latest && (
        <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-4"><h3 className="mb-1 text-sm font-bold text-white">Último partido vs perfil competitivo</h3><p className="mb-3 text-[11px] text-zinc-500">Perfil calculado con {profileMatches} partidos de más de 80 minutos.</p><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{KPI_KEYS.map((key) => { const def = REPORT_METRICS.find((item) => item.key === key); const delta = pctVs(latest.gpsRow?.[key], profileValue(key)); return <div key={key} className="flex items-center justify-between rounded-lg bg-zinc-950/60 p-2.5"><span className="text-xs text-zinc-400">{def.label}</span><span className={`text-xs font-black ${tone(delta)}`}>{delta > 0 ? "+" : ""}{delta}%</span></div>; })}</div></div>
      )}
