    ...
    <div className="rounded-2xl border border-zinc-800 bg-zinc-900/70 p-4">
      ...
      {filters.dateRange === "custom" && (
        <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-3">
          ...
          <FilterActions onReset={resetFilters} className="items-end justify-start md:justify-end" />
        </div>
      )}
      {filters.dateRange !== "custom" && (
        <FilterActions onReset={resetFilters} className="mt-3 justify-end" />
      )}
    </div>
  );
}

function FilterActions({ onReset, className = "" }) {
  return (
