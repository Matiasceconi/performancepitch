...
export default function GpsMicrocycleWeekNavigator({ weeklyPlans = [], selectedPlanId, onSelectPlan, sessions = [], gpsBySession = {} }) {
  ...
  const currentPlan = sorted.find((p) => p.week_start === moment(today).startOf("isoWeek").format("YYYY-MM-DD") && (p.days_data || []).some((d) => d.date === today)) || sorted.find((p) => (p.days_data || []).some((d) => d.date === today));
  ...
  return <div className="flex flex-wrap items-center gap-3">
    ...
    <button onClick={() => currentPlan && onSelectPlan(currentPlan.id)} className="h-10 rounded-xl border border-zinc-700/80 bg-zinc-950/80 px-4 text-sm font-semibold text-zinc-300 hover:border-zinc-600 hover:text-white">Ir a semana actual</button>
