...
const DEFAULT_OPTIONS = {
  ...
  metricKeys: MICRO_METRICS.slice(0, 9).map((metric) => metric.key),
};

export default function GpsMicrocyclePdfButton({ squadName, season, dailySummaries, highlights, comparison, cycleDays, selectedDates = [], visibleMetrics = MICRO_METRICS, rankingConfig, matchContext, cycleRows = [] }) {
  const [open, setOpen] = useState(false);
  const [preview, setPreview] = useState(false);
  const [exporting, setExporting] = useState(false);
  const [options, setOptions] = useState(DEFAULT_OPTIONS);
  const selectedMetricObjects = useMemo(() => MICRO_METRICS.filter((metric) => (options.metricKeys || []).includes(metric.key)), [options.metricKeys]);
  ...
  async function exportPdf() {
    ...
    try {
      ...
      await generateMicrocyclePdf({ squadName, season, dailySummaries: exportDays, highlights, comparison, metrics: selectedMetricObjects, cycleDays, selectedDates, rankingConfig, matchContext, cycleRows, options, aiText });
      setOpen(false);
    } finally { setExporting(false); }
  }

  function toggleMetric(key) {
    setOptions((current) => ({ ...current, metricKeys: current.metricKeys.includes(key) ? current.metricKeys.filter((item) => item !== key) : [...current.metricKeys, key] }));
  }
