...
import { fetchFootballData } from "@/components/futbol/useFootballData";
import { fixtureToPdfMatchContext, getNextReservaFixture } from "@/lib/nextReservaMatch";
...
export default function GpsMicrocyclePdfButton({ squadName, season, dailySummaries, highlights, comparison, cycleDays, selectedDates = [], visibleMetrics = MICRO_METRICS, chartMetrics = [], chartConfig, rankingConfig, cycleRows = [], playerMap = {} }) {
  ...
  async function loadNextReservaMatch() {
    try {
      const [footballData, competitions] = await Promise.all([
        fetchFootballData(),
        base44.entities.Competitions.list("name", 200),
      ]);
      const clubName = clubBrand?.name || "Defensa y Justicia";
      const fixture = getNextReservaFixture({
        fixtures: footballData?.fixtures || [],
        competitions,
        teamName: clubName,
      });
      return fixtureToPdfMatchContext(fixture, clubName);
    } catch (error) {
      console.error("No se pudo resolver el próximo partido de Reserva para el PDF", error);
      return null;
    }
  }
  ...
  async function exportPdf() {
    setExporting(true);
    try {
      const [aiText, nextMatchContext] = await Promise.all([buildAiText(), loadNextReservaMatch()]);
      await generateMicrocyclePdf({
        squadName,
        season,
        clubName: clubBrand?.name || "Defensa y Justicia",
        clubLogoUrl: clubBrand?.logoUrl || "",
        dailySummaries: exportDays,
        highlights,
        comparison,
