...
import { findMatchForDay, normalizeMatchGpsRows } from "./matchGpsAdapter";
...
const OPTIONAL = [{ key: "acc_2_3", label: "Acc 2–3 m/s²" }, { key: "dec_2_3", label: "Dec 2–3 m/s²" }, { key: "player_load_per_min", label: "Player Load/min" }, { key: "hmld", label: "HMLD" }, { key: "rhie_bouts", label: "RHIE" }, { key: "smax_pct", label: "% Vel. máxima", mode: "max" }, { key: "gps_days", label: "Días con GPS" }, { key: "days_without_data", label: "Días sin datos" }];
...
export default function GpsDailyPlayerTable({ selectedDates = [], sessions = [], gpsBySession = {}, matchReports = [], matchGpsByMatch = {}, playerMap = {}, microcycleProfiles = [], competitionProfiles = [], squadId, season, selectedDays = [], totalDays = 0 }) {
  ...
  const restDateSet = useMemo(() => new Set((selectedDays || []).filter((day) => day.day_type === "rest" || day.is_rest_day === true).map((day) => day.date)), [selectedDays]);
  const periodSessions = sessions.filter((s) => selectedSet.has(s.date) && !restDateSet.has(s.date));
  const periodMatches = useMemo(() => (selectedDays || []).filter((day) => selectedSet.has(day.date) && day.day_type === "match").map((day) => {
    const match = findMatchForDay(day, matchReports);
    if (!match) return null;
    return { id: match.id, title: `Partido vs ${match.rival || "Rival"} — MD`, date: match.date, match_day_code: "MD", microcycle_day: "MD", source_type: "match" };
  }).filter(Boolean), [selectedDays, selectedSet, matchReports]);
  const periodItems = [...periodSessions, ...periodMatches];
  const gpsByItem = useMemo(() => {
    const normalized = {};
    Object.entries(matchGpsByMatch || {}).forEach(([matchId, rows]) => {
      const match = matchReports.find((m) => m.id === matchId);
      const day = (selectedDays || []).find((d) => d.match_id === matchId || (match && d.date === match.date && d.day_type === "match")) || {};
      normalized[matchId] = normalizeMatchGpsRows(match, rows, day);
    });
    return { ...gpsBySession, ...normalized };
  }, [gpsBySession, matchGpsByMatch, matchReports, selectedDays]);
  const activeSessions = sessionId === "all" ? periodItems : periodItems.filter((s) => s.id === sessionId);
  const metrics = groups.flatMap((g) => g.metrics.map((m) => ({ ...m, group: g.label })));
  const rows = useMemo(() => aggregateRows(activeSessions, gpsByItem, playerMap, metrics, selectedDates, viewMode), [activeSessions, gpsByItem, playerMap, metrics, selectedDates, viewMode]);
  ...
  const title = selectedDates.length === 1 && periodMatches.length === 1 && periodSessions.length === 0 ? periodMatches[0].title : selectedDates.length === 1 ? `Jugadores del ${moment(selectedDates[0]).locale("es").format("dddd")} – ${selectedDays[0]?.md || "—"}` : totalDays > 0 && selectedDates.length === totalDays ? "Carga acumulada del microciclo" : `Carga acumulada de ${selectedDates.length} días`;
  const onlyRestSelected = selectedDates.length > 0 && selectedDates.every((date) => restDateSet.has(date));
  ...
  function ownRef(row, metricKey) { const key = REF_KEYS[metricKey]; if (!key) return { value: null, source: "Sin referencia", used: "" }; const profiles = selectedDays.map((day) => microMap[`${row.player_id}:${day.md || day.match_day_code}`]).filter(Boolean); const values = profiles.map((p) => Number(p[key])).filter(Number.isFinite); if (!values.length) return { value: null, source: "Sin referencia", used: "" }; const value = MAX_KEYS.has(metricKey) ? Math.max(...values) : metricKey === "m_min" ? values.reduce((a, b) => a + b, 0) / values.length : values.reduce((a, b) => a + b, 0); const sessionsUsed = profiles.reduce((a, p) => a + Number(p.sessions_used || 0), 0); return { value: viewMode === "avg" && SUM_KEYS.has(metricKey) ? value / Math.max(1, selectedDates.length) : value, source: `Perfil propio de los días`, used: `${sessionsUsed} sesiones históricas de ${selectedDays.map((d) => d.md).filter(Boolean).join(", ")}` }; }
...
function aggregateRows(sessions, gpsBySession, playerMap, metrics, selectedDates, viewMode) {
  ...
  return Object.values(map).map((item) => {
    ...
    metrics.forEach((m) => {
      ...
      else if (m.key === "sessions_count") out[m.key] = item.sessionIds.size;
      else if (m.key === "gps_days") out[m.key] = item.gpsDates.size;
      else if (m.key === "days_without_data") out[m.key] = Math.max(0, selectedDates.length - item.gpsDates.size);
      ...
      if (viewMode === "avg" && SUM_KEYS.has(m.key) && item.sessionIds.size) out[m.key] = out[m.key] / item.sessionIds.size;
    });
    return out;
  });
}
