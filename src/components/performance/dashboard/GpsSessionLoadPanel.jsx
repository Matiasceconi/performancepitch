...
import { AlertCircle, Upload, Users, Clock, Target, Activity } from "lucide-react";
...
export default function GpsSessionLoadPanel({ sessions, gpsBySession, matchGpsByMatch = {}, playerMap, squadName, season, squadId, competitionProfiles = [], microcycleProfiles = [], matchReports = [], onReload: _onReload }) {
  ...
  useEffect(() => {
    if (selectedSessionId && sortedSessions.some((s) => s.id === selectedSessionId)) return;
    const withGps = sortedSessions.filter((s) => (gpsBySession[s.id] || []).some((r) => r.include_in_session_average !== false));
    if (withGps.length) setSelectedSessionId(withGps[0].id);
    else if (sortedSessions.length) setSelectedSessionId(sortedSessions[0].id);
  }, [sortedSessions, gpsBySession, selectedSessionId]);

  const selectedSession = sortedSessions.find((s) => s.id === selectedSessionId);
  const allSessionGpsRows = gpsBySession[selectedSessionId] || [];
  const includedRows = allSessionGpsRows.filter((r) => r.include_in_session_average !== false);
  const hasGps = includedRows.length > 0;
  const statusInfo = selectedSession ? gpsStatus(selectedSession, gpsBySession) : null;
  ...
  const sessionSource = useMemo(() => {
    ...
    return allGpsSources.find((s) => s.sourceId === selectedSessionId && s.sourceType === "training") || null;
  }, [allGpsSources, selectedSessionId]);

  const previousSession = useMemo(() => {
    if (!selectedSession) return null;
    return sortedSessions.find((s) => s.date < selectedSession.date && (gpsBySession[s.id] || []).some((r) => r.include_in_session_average !== false)) || null;
  }, [sortedSessions, selectedSession, gpsBySession]);
  ...
  const periodSources = sessionSource ? [sessionSource] : [];
  const previousSources = previousSource ? [previousSource] : [];
  const selectedSourceIds = sessionSource ? [sessionSource.id] : [];
  ...
  return (
    <div className="space-y-5">
      ...
      {selectedSession && (
        <div className="rounded-2xl border border-zinc-800 bg-zinc-900/70 p-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <h3 className="text-lg font-black text-white">{selectedSession.title || `Sesión ${selectedSession.session_number || ""}`}</h3>
              <p className="text-sm text-zinc-400 capitalize">{moment(selectedSession.date).format("dddd DD/MM/YYYY")}</p>
            </div>
            {statusInfo && (
          ...
          <div className="mt-3 flex flex-wrap gap-x-6 gap-y-2 text-sm">
            {selectedSession.start_time && (
              <div className="flex items-center gap-1.5 text-zinc-300">
                ...
                <span>{selectedSession.start_time}{selectedSession.end_time ? ` - ${selectedSession.end_time}` : ""}</span>
              </div>
            )}
            ...
            {selectedSession.session_objective && (
              <div className="flex items-center gap-1.5 text-zinc-300">
                <Target size={14} className="text-zinc-500" />
                <span className="text-zinc-500">Objetivo:</span>
                <span>{selectedSession.session_objective}</span>
              </div>
            )}
            <div className="flex items-center gap-1.5 text-zinc-300">
      ...
      {/* No GPS state */}
      {selectedSession && !hasGps && (
        <div className="rounded-2xl border border-amber-500/30 bg-amber-500/5 p-8 text-center">
          <AlertCircle size={32} className="mx-auto text-amber-400 mb-3" />
          <h3 className="text-lg font-bold text-white">Esta sesión todavía no tiene datos GPS</h3>
          <p className="mt-2 text-sm text-zinc-400">La sesión permanece seleccionada. Procesá el archivo GPS para ver los datos.</p>
