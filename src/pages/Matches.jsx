...
function getLogoFromClubs(name, clubs) {
  ...
  // Match exacto por rival_club_id ya viene en match.rival_logo_url.
  // Acá buscamos por nombre normalizado como fallback.
  const byName = clubs.find(c => normalizeStr(c.official_name) === key || normalizeStr(c.short_name) === key);
  if (byName?.shield_url) return byName.shield_url;
  // Match parcial por alias
  const byAlias = clubs.find(c => (c.aliases || []).some(a => normalizeStr(a) === key));
  if (byAlias?.shield_url) return byAlias.shield_url;
  return null;
}
...
// ── MatchCard ─────────────────────────────────────────────────────────────────
function MatchCard({ match, players, onEdit, onDelete, onMatchUpdated, squadId, competitions, competitionMap, rivalClubs }) {
