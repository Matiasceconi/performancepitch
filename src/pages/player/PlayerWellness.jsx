...
const PAIN_ZONES = ['Sin dolor', 'Cuello', 'Hombro', 'Espalda alta', 'Espalda baja', 'Cadera', 'Islaquiotibial', 'Cuádriceps', 'Gemelo', 'Tobillo', 'Rodilla', 'Pie', 'Otro'];

const STEPS = [
  { key: 'sleep_hours', type: 'hours', label: '¿Cuántas horas dormiste?', subtitle: 'Horas de sueño' },
  { key: 'sleep_quality', type: 'scale15', label: '¿Cómo fue la calidad de tu sueño?', subtitle: '1 · Pregunta 2 de 10' },
...
export default function PlayerWellness() {
  ...
  const totalSteps = STEPS.length + 3; // + pain_zone, pain_intensity, comment
  ...
  async function submit() {
    ...
    setError('');
    try {
      const res = await base44.functions.invoke('submitWellness', values);
  ...
  return (
    <div className="p-5 space-y-4 min-h-screen flex flex-col">
      <div className="flex items-center gap-3">
        {step > 0 && <button onClick={back} className="p-2 -ml-2 text-zinc-400 hover:text-white"><ChevronLeft size={22} /></button>}
        ...
        {step === STEPS.length && values.has_pain && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              {PAIN_ZONES.filter((z) => z !== 'Sin dolor').map((z) => (
